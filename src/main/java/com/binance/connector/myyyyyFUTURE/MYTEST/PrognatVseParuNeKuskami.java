...
public class PrognatVseParuNeKuskami { //todo  в классе SvechaParser поменяфайл - ВЕРНУТЬ ОБРАТНО  и class ParserNazvanyi {
    ...
    public static void main(String[] args) {
        ...
        List<String> vseNazvyanyyaVListe = ParserNazvanyi.parsNazvanya();     //todo  в классе SvechaParser поменяфайл - ВЕРНУТЬ ОБРАТНО  и class ParserNazvanyi {
...
//        System.out.println("Баланс от изначального 100 долларо спустя годовой прогон по всем парам = " + balance);
    ...
    public static void processSymbol(String symbol) {
        ...
        List<Svecha> listVsehSvecheyOdnoyMonety = SvechaParser.parsimZaGod(symbol); //todo  в классе SvechaParser поменяфайл - ВЕРНУТЬ ОБРАТНО  и class ParserNazvanyi {


        prostavitSvechamSma(listVsehSvecheyOdnoyMonety);
        ...
        long startTime = System.currentTimeMillis(); // Записываем время перед выполнением метода

        ZAPUSKREALNOGOTESTA(listVsehSvecheyOdnoyMonety, symbol); //todo прогонка всех данных

        long endTime = System.currentTimeMillis(); // Записываем время после выполнения метода

        long duration = (endTime - startTime); // Разница даст вам время выполнения в миллисекундах

        System.out.println("Время выполнения метода ZAPUSKREALNOGOTESTA: " + duration / 1000.0 + " секунд.");
